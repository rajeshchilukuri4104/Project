...
def terminal_size():
    try:
        # Use subprocess.check_output to get the output of 'stty size' command
        ...
        # Return a default value if there is an error or the output is not valid
        return 20
...
def url_maker(url):
    ...
    host = parsed.netloc
    # Remove 'www.' from the beginning of the host, if present
...
(Author :By Bhanu)
      ...
      {bcolors.OKGREEN}Explore vulnerabilities in web applications.{bcolors.ENDC}
    """
    print(logo_ascii)
    print(bcolors.ENDC)
            ...
            while self.busy:
                if not self.disabled:
                    ...
                    if inc>random.uniform(0,terminal_size()): #30 init
                        print(end="\r")
                        bcolors.BG_SCAN_TXT_START = '\x1b[6;30;' + str(round(random.uniform(47, 47))) + 'm' 
                        inc = 0
                    sys.stdout.flush()
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/probeforweb_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/probeforweb_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/probeforweb_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/probeforweb_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/probeforweb_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o probeforweb_temp_wapiti"],
...
# Tool Set
tools_precheck = [
                    ["wapiti"], ["whatweb"], ["nmap"], ["golismero"], ["host"], ["wget"], ["uniscan"], ["wafw00f"], ["dirb"], ["davtest"], ["theHarvester"], ["xsser"], ["dnsrecon"],["fierce"], ["dnswalk"], ["whois"], ["sslyze"], ["lbd"], ["golismero"], ["dnsenum"],["dmitry"], ["davtest"], ["nikto"], ["amass"]
                 ]
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("Probeforweb is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum probeforweb.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/skavngr/probeforweb/master/probeforweb.py -O probeforweb.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of Probeforweb." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"Probeforweb successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    os.system('rm /tmp/probeforweb* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"Probeforweb was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by Probeforweb."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. Probeforweb will still perform the rest of the tests. Install these tools to fully utilize the functionality of Probeforweb."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/probeforweb_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                elapsed = scan_stop - scan_start
                rs_total_elapsed = rs_total_elapsed + elapsed
                #sys.stdout.write(MOVE_CURSOR_UP_ONE_LINE) 
                sys.stdout.write(ERASE_CURRENT_LINE)
                #print("-" * terminal_size(), end='\r', flush=True)
                print(bcolors.OKBLUE+"\nScan Interrupted in "+display_time(int(elapsed))+bcolors.ENDC, end='\r', flush=True)
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit Probeforweb.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    #             report.write("\n------------------------\n\n")
    #             temp_report_name = "/tmp/probeforweb_temp_"+vuln_info[arg1]
    ...
    #         print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory Probeforweb resides.")
    ...
    #         try:
    #             with open("/tmp/probeforweb_temp_"+file_name[arg1], 'r') as temp_report:
    #                     data = temp_report.read()
    ...
    # os.system('rm /tmp/probeforweb_te* > /dev/null 2>&1') # Clearing previous scan files

def generate_vulnerability_html(vulnerability_info, temp_report_name):
    html = f"<h2>{vulnerability_info[arg2]}</h2>\n"
    html += "<pre>\n"
    with open(temp_report_name, 'r') as temp_report:
        data = temp_report.read()
        html += data
...
def generate_html_report(target, rs_vul_list, arg1, arg2, tool_names, rs_skipped_checks, rs_total_elapsed):
    date = subprocess.Popen(["date", "+%Y-%m-%d"], stdout=subprocess.PIPE).stdout.read()[:-1].decode("utf-8")
    ...
    if len(rs_vul_list) == 0:
        ...
        html_report += "<ul>\n"
        for rs_vul, vuln_info in enumerate(rs_vul_list):
            vuln_info = vuln_info.split('*')
            temp_report_name = f"/tmp/probeforweb_temp_{vuln_info[arg1]}"
            html_report += "<li>\n"
    ...
    html_report += "<p>Total Number of Vulnerability Checks: <strong>{len(tool_names)}</strong></p>\n"
    html_report += "<p>Total Number of Vulnerability Checks Skipped: <strong>{rs_skipped_checks}</strong></p>\n"
    html_report += "<p>Total Number of Vulnerabilities Detected: <strong>{len(rs_vul_list)}</strong></p>\n"
    html_report += "<p>Total Time Elapsed for the Scan: <strong>{display_time(int(rs_total_elapsed))}</strong></p>\n"
    html_report += "<p>For Debugging Purposes, You can view the complete output generated by all the tools <a href='{debuglog}'>{debuglog}</a> under the same directory.</p>\n"
...
generate_html_report(target, rs_vul_list, arg1, arg2, tool_names, rs_skipped_checks, rs_total_elapsed)
